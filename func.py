...
class FunctionBot: 
    ...
    def __init__(self, link): 
        ...
        options.add_argument("--proxy-server=212.115.232.79:31280")
    ...
    def get_into_the_test(self, name_for_test="ㅤㅤㅤ ㅤㅤ"):
        ...
        self.file_question = CreateFile(self.name_of_the_test)
...
class CreateFile: 
    ...
    def __init__(self, name_of_the_test): 
        self.name_of_the_test = name_of_the_test
        self.count = 1 
        with open(f"vseobot-question.txt", "w", encoding="utf-8") as file:
            file.write("Done by vseobot 0.1 (linwexEU)\n")

    def add_question(self, type_question, question): 
        with open(f"{self.name_of_the_test}.txt", "a", encoding="utf-8") as file:
            # З ОДНІЄЮ ПРАВИЛЬНОЮ ВІДПОВІДДЮ | З КІЛЬКОМА ПРАВИЛЬНИМИ ВІДПОВІДЯМИ 
...
if __name__ == "__main__":
    fc = FunctionBot("https://vseosvita.ua/test/go-settings?code=azb083")
